...
export async function scrapeSmartChart(url) {
  const client = new ZenRows(process.env.API_KEY);
    
  try{
    const { data } = await client.get(process.env.URL, {
      "js_render": "true",
      "js_instructions": "[{\"fill\": \[\"input[id='edit-name']\", \"meredith@meredithfogle.com\"]}]"
  });
